...
function Agent() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        setUsername(cachedUsername || t("guest"));
        setPhoto(cachedPhoto || "/default-profile.png");
        ...
        if (res.data) {
          ...
          // Update cache
          localStorage.setItem("username", res.data.name || t("guest"));
          if (res.data.photo) {
            localStorage.setItem("photo", getImageUrl(res.data.photo));
          }
        }
      } catch (error) {
        console.warn("User fetch failed, using cached data");
      }
    };
  ...
  // ================= APPROVED FARMERS (Optimized) =================
  useEffect(() => {
    if (!agentId) return;

    const fetchApprovedFarmers = async () => {
      try {
        const res = await api.get(`/api/access/approved-farmers/${agentId}`);
        if (res.data) {
          setSharedFarmers(res.data || []);
        }
      } catch (error) {
        console.error("Failed to fetch approved farmers:", error);
        setSharedFarmers([]);
      }
    };
  ...
  const openAgent = useCallback(async (agentId, agentName) => {
    ...
    try {
      ...
      if (res.data) {
        ...
        // Pre-check access for all farmers (batch request if possible)
        // If API supports batch check, use that instead
        // For now, we'll only check when user interacts
      }
    } catch (error) {
  ...
  // ================= REQUEST FARMER ACCESS (Optimized) =================
  const requestFarmerAccess = useCallback(async (farmerId) => {
    setLoadingAccess(prev => ({ ...prev, [farmerId]: true }));
    try {
      const res = await api.post("/api/access/request-farmer", {
        requesterId: userId,
  ...
  // ================= CHECK FARMER ACCESS (Optimized) =================
  const checkFarmerAccess = useCallback(async (farmerId) => {
    setLoadingAccess(prev => ({ ...prev, [farmerId]: true }));
    try {
      const res = await api.get(
        `/api/access/check-farmer?requesterId=${userId}&farmerId=${farmerId}`
      ...
      console.error("Failed to check farmer access:", error);
      alert(t("errorCheckingAccess"));
    } finally {
      setLoadingAccess(prev => ({ ...prev, [farmerId]: false }));
    }
  ...
  useEffect(() => {
    ...
    loadNotifications();
    pollRef.current = setInterval(loadNotifications, 30000); // Increased to 30 seconds
  ...
  }, [farmers, isLoadingFarmers, farmerAccess, loadingAccess, selectedAgentName, t, requestFarmerAccess, checkFarmerAccess]);
    ...
    <div className="agent-page-container">
      ...
      <div className={`agent-content-area ${isMobile ? 'mobile-view' : ''}`}>
        ...
        <div className="agent-header">
          ...
          <div className="agent-header-right">
            {/* Notifications removed for optimization */}
          </div>
        </div>
        ...
        <div className="agent-user-section">
          <div className="agent-user-header">
            <h4 className="agent-user-title">{t("allRegisteredUsers")}</h4>
            <span className="agent-user-count">
              {isLoadingUsers ? (
          ...
          <div className="agent-list-grid">
            {isLoadingUsers ? (
              ...
              <div className="agent-empty-state">
                <div className="agent-empty-icon">👤</div>
                <p className="agent-empty-text">{t("noUsersFound")}</p>
              </div>
            ) : (
              UserList
            )}
