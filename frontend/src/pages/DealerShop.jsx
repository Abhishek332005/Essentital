...
const DealerShop = () => {
  ...
  const getDiscountInfo = (product) => {
    // If product has explicit discountBadge, use it
    if (product.discountBadge) {
      return {
        show: true,
        text: product.discountBadge,
        type: 'badge' // badge, discount, offer, etc.
      };
    }
    
    // Alternative: Calculate discount from variants if needed
    // Example logic: if any variant has discountPercentage property
    if (product.variants && product.variants.length > 0) {
      const variantWithDiscount = product.variants.find(v => v.discountPercentage);
      if (variantWithDiscount) {
        return {
          show: true,
          text: `${variantWithDiscount.discountPercentage}% OFF`,
          type: 'discount'
        };
      }
    }
      ...
      if (existingItem) {
        ...
        updatedCart = [
          ...cart,
          {
            ...
            quantity: 1,
            weightValue: defaultVariant.label,   // ✅ ADDED: SAVE "10kg" / "20kg"
            discountBadge: product.discountBadge // ✅ Save discount badge info
          },
        ];
      }
        ...
        <div className="products-grid">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => {
              ...
              return (
                <div key={product.id} className="product-card">
                  {/* ✅ DISCOUNT BADGE ADDED HERE */}
                  {discountInfo.show && (
                    <div className={`discount-badge discount-${discountInfo.type}`}>
                      {discountInfo.text}
                    </div>
                  )}
                  ...
                  <button
                    onClick={() => addToCart(product)}
                    disabled={addingId === product.id}
                    className={discountInfo.show ? "has-discount" : ""}
                  >
                    {addingId === product.id
                      ? "Adding..."
                      : addedId === product.id
